var text = require('./text'), ReParse = require('reparse').ReParse;
...
function foldHeader(name, value, safe) {
  ...
  segments = text.wrap(line, 78, 998, ".(?=\\s)"), result = segments[0];
...
function fill(body) {
  ...
  var reducer = function(sub) {
    return text.splitLines(sub).reduce(wrap, []).join('\r\n');
  };
  // (2020.07.30 sakamoto) 添付ファイル用に Array 形式をサポート
  // https://www.w3.org/Protocols/rfc1341/7_2_Multipart.html
  if(Array.isArray(body)) {
    return body.map(t=>!t || t.indexOf('--') == 0 ? t: reducer(t)).join('\r\n');
  } else {
    return reducer(body);
  }
...
function titleCaseHeader(name) {
  return name && name.split(/\-/).map(function(segment) {
    if(/^ID$/i.test(segment))
      return 'ID';
    if(/DKIM/i.test(segment))
      return 'DKIM';
...
var DAY = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'], MONTH = ['Jan',
  'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];

function date(when) {
  when = when || new Date();
  return (DAY[when.getDay()] + ', ' + pad(when.getDate()) + ' '
    + MONTH[when.getMonth()] + ' ' + when.getFullYear() + ' '
    + pad(when.getHours()) + ':' + pad(when.getMinutes()) + ':'
    + pad(when.getSeconds()) + ' ' + tz(when.getTimezoneOffset()));
}
