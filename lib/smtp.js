...
Client.prototype.mail = function(from, to) {
  ...
  this._domain = this._domain || OS.hostname();
...
Client.prototype.connect = function() {
  var cl = this, opts = cl.options || {};
  var sock = cl.sock;

  var connect_error = function(e) {
    clearTimeout(connect_timer);
    sock.destroy();
    cl.emit('error', e);
  };
  var connect_timer = setTimeout(function(){
    connect_error(new Error('Timeout to connect'));
  }, opts.connect_timeout || 30 * 1000);

  if (!cl._domain)
    connect_error(new Error('Missing required domain.'));
  else if (cl.sock.readyState != 'closed')
    connect_error(new Error('Session already started.'));
  else {
    sock.once('error', connect_error);
    sock.once('connect', function() {

        sock.removeListener('error', connect_error);
        cl.reset(220);

    });
    sock.connect(cl.port, cl.host);
...
Client.prototype.authLOGIN = function(username, password, next) {
  ...
  function sendUsername() {
    var buf = Buffer.from ? Buffer.from(username): new Buffer(username);
    self.puts(buf.toString('base64'))
      .withReply(334, sendPassword);
  }

  function sendPassword() {
    var buf = Buffer.from ? Buffer.from(password): new Buffer(password);
    self.puts(buf.toString('base64'))
      .withReply(235, next);
  }
